...
module Formattable
  ...
  def simple_blurbify(text, allowed_tags = NB.allowed_html_tags)
    return '' if text.blank?
    text = ActionController::Base.helpers.sanitize(text, { tags: allowed_tags.split(/, ?| /) + ['span'] }) # REVIEW: Why add <span> here? For Feature titles
    text = clean_whitespace(text)
  ...
  def annotate(text)
    ...
    if !annotations.empty?
      ...
      wrapped_annotations = annotations.flatten.each_with_index.map do |annotation, index|
      ...
      return "<section class=\"body\">#{text.html_safe}</section><section id=\"annotations\"><h3>Annotations</h3><ol>#{wrapped_annotations.join}</ol></section>"
    end
    # render 'notes/text.html', text: text
    return "<section class=\"body\">#{text.html_safe}</section>"
  end
